...
@app.get('blog/{check}',status_code=status.HTTP_200_OK)

def status_checket(check:int,response:Response):
    if id>5:
        response.status_code=status.HTTP_404_NOT_FOUND
        return{'error':f'The {check} is grater than 5'}
    else:
      response.status_code=status.HTTP_200_OK
      return{'message':f'The {check} is not grater than 5'}   
